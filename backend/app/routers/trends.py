...
@router.post("/update/")
async def update_and_get_trends(categories: List[str], db: AsyncSession = Depends(async_get_db)):
    """
    트렌드 데이터를 업데이트하고 결과를 반환하는 엔드포인트.
    """
    try:
        # 트렌드 업데이트
        await update_trends(db, categories)
        
        # 업데이트된 트렌드 조회
        result = await db.execute(select(Trend).filter(Trend.category.in_(categories)))
